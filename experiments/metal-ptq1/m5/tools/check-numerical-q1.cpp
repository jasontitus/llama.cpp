...
static bool check(ggml_backend_t backend, int k, int m, bool padded, int pattern) {
    ...
    std::vector<uint8_t> packed(ggml_row_size(GGML_TYPE_Q1_0, k)*m);
    ggml_quantize_chunk(GGML_TYPE_Q1_0, weights.data(), packed.data(), 0, m, k, nullptr);
    ggml_get_type_traits(GGML_TYPE_Q1_0)->to_float(packed.data(), dequant.data(), k*m);
    ...
    for (int n : {1, 2, 3, 4, 5, 6, 7, 8}) {
        ...
        for (int rep = 0; rep < repeats; ++rep) {
            ...
            ggml_tensor * a = ggml_new_tensor_2d(ctx, GGML_TYPE_Q1_0, k, m);
