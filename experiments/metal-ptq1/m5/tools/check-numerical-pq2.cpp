...
static bool check(ggml_backend_t backend, int k, int m, bool padded, int pattern) {
    ...
    std::vector<uint8_t> packed(ggml_row_size(GGML_TYPE_PQ2_0, k)*m);
    ggml_quantize_chunk(GGML_TYPE_PQ2_0, weights.data(), packed.data(), 0, m, k, nullptr);
    ggml_get_type_traits(GGML_TYPE_PQ2_0)->to_float(packed.data(), dequant.data(), k*m);
...
int main() {
    ...
    for (int k : {128, 384, 512, 5120, 17408}) {
        for (int m : {1, 3, 7, 8, 4163}) {
            for (bool padded : {false, true}) { ok &= check(backend, k, m, padded, 0); }
        }
    }
    for (int pattern : {1, 2}) { ok &= check(backend, 5120, 7, true, pattern); }
    // K past 32767: PQ2_0 staging offsets once overflowed a short
    for (bool padded : {false, true}) { ok &= check(backend, 33024, 67, padded, 0); }
