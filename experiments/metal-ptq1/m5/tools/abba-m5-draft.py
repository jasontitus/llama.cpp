#!/usr/bin/env python3
"""ABBA study of two environment/draft-length arms on one binary: llama-bench cases and fresh-server cells.
...
p.add_argument('--draft-a', type=int, default=1, help='MTP draft tokens for arm A (0 = plain decoding)')
p.add_argument('--draft-b', type=int, default=1, help='MTP draft tokens for arm B')
a = p.parse_args()
...
def server_obs(cell, arm, cycle):
    ...
    # draft 0 = plain decoding on the same merged model (its MTP head is simply unused)
    draft = a.draft_a if arm == 'A' else a.draft_b
    cmd += ['--spec-type', 'draft-mtp', '--spec-draft-n-max', str(draft), '--spec-draft-n-min', '0', '--spec-draft-p-min', '0'] if spec and draft > 0 else ['--spec-type', 'none']
    ...
    try:
        ...
        acc = [(r['timings'].get('draft_n', 0), r['timings'].get('draft_n_accepted', 0)) for r in rows]
        return dict(tps=agg, gen_tps_mean=gen, wall_s=span, requests=rows, command=cmd, drafts=acc)
    finally:
...
def quartet(cell, cycle):
    ...
    for attempt in range(1, a.attempts + 1):
        ...
        for pos, arm in enumerate('ABBA'):
            ...
            if 'drafts' in r: say(f'    drafts (n, accepted) per slot: {r["drafts"]}')
            obs.append(r)
